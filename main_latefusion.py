...
from late_fusion_net.data_parser import load_data
from late_fusion_net.dataset import MuSeDataset, custom_collate_fn
from late_fusion_net.eval import evaluate, calc_ccc, calc_auc, mean_pearsons
from loss import CCCLoss, BCELossWrapper, MSELossWrapper
from late_fusion_net.model import Model
from late_fusion_net.train import train_model
...
def parse_args():
    ...
    parser.add_argument('--task', type=str, choices=TASKS,default=PERSONALISATION,
                        help=f'Specify the task from {TASKS}.')
    parser.add_argument('--feature', nargs='+', default=['apvit', 'w2v-msp'],
    ...
    # parser.add_argument('--model_dim_0', type=int, default=128,
    #                     help='Specify the number of hidden states in the RNN (default: 64).')
    # parser.add_argument('--rnn_n_layers_0', type=int, default=4,
    #                     help='Specify the number of layers for the RNN (default: 1).')
    # parser.add_argument('--rnn_bi_0', action='store_true',
    #                     help='Specify whether the RNN is bidirectional or not (default: False).')
    # parser.add_argument('--d_fc_out_0', type=int, default=64,
    #                     help='Specify the number of hidden neurons in the output layer (default: 64).')
    # parser.add_argument('--rnn_dropout_0', type=float, default=0.5)
    # parser.add_argument('--linear_dropout_0', type=float, default=0.5)
    #
    # parser.add_argument('--model_dim_1', type=int, default=32,
    #                     help='Specify the number of hidden states in the RNN (default: 64).')
    # parser.add_argument('--rnn_n_layers_1', type=int, default=4,
    #                     help='Specify the number of layers for the RNN (default: 1).')
    # parser.add_argument('--rnn_bi_1', action='store_true',default=True,
    #                     help='Specify whether the RNN is bidirectional or not (default: False).')
    # parser.add_argument('--d_fc_out_1', type=int, default=64,
    #                     help='Specify the number of hidden neurons in the output layer (default: 64).')
    # parser.add_argument('--rnn_dropout_1', type=float, default=0.5)
    # parser.add_argument('--linear_dropout_1', type=float, default=0.5)
    #
    # parser.add_argument('--fusion_size', type=int, default=64,
    #                     help='Specify the number of hidden states in the RNN (default: 64).')
    # parser.add_argument('--fusion_n_layers', type=int, default=1,
    #                     help='Specify the number of layers for the RNN (default: 1).')
    # parser.add_argument('--fusion_bi', action='store_true',default=True,
    #                     help='Specify whether the RNN is bidirectional or not (default: False).')
    # parser.add_argument('--fusion_dropout', type=float, default=0.2)
    # parser.add_argument('--fusion_hidden', type=int, default=32)
    # parser.add_argument('--fusion_fc_drop', type=float, default=0.2)
...
def main(args):
    ...
    datasets = {partition:MuSeDataset(data, partition) for partition in data[0].keys()}


    for idx,i in enumerate(datasets['train'].get_feature_dim()):
        setattr(args,f"d_in_{idx}",i)
    ...
    if args.eval_model is None:  # Train and validate for each seed
        ...
        for seed in seeds:
            ...
            for i in range(len(model.submodels)):
                if getattr(args, f"model_path_{i}"):
                    model.submodels[i] = torch.load(getattr(args, f"model_path_{i}"))
            print('=' * 50)
...
if __name__ == '__main__':
    ...
    if args.config_path:
        ...
        for k, v in cfg.items():
            if k != "model":
                setattr(args, k, v)
        submodel = cfg["model"]["submodel"]
        for i in range(len(submodel)):
            for k,v in submodel[i].items():
                setattr(args, f"{k}_{i}", v)
        for k, v in cfg["model"]["fusion_model"].items():
            setattr(args, k, v)

    args.log_file_name = '{}_{}_[{}]_[{}]_[{}_{}_{}_{}]_[{}_{}]'.format('RNN',
        datetime.now(tz=tz.gettz()).strftime("%Y-%m-%d-%H-%M"), ','.join(args.feature), args.emo_dim,
        args.model_dim_0, args.rnn_n_layers_0, args.rnn_bi_0, args.d_fc_out_0, args.lr, args.batch_size) if args.task == PERSONALISATION else \

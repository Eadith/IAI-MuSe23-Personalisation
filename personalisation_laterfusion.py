...
from late_fusion_net.data_parser import load_personalisation_data
from late_fusion_net.dataset import MuSeDataset, custom_collate_fn
from late_fusion_net.eval import get_predictions
from main import get_eval_fn, get_loss_fn
from late_fusion_net.train import train_personalised_models
from utils import seed_worker, log_results


def parse_args():
    parser = ArgumentParser()
    parser.add_argument('--model_id', help='model id',default="RNN_2023-06-02-14-55_[['w2v-msp','apvit']]_[valence]_[128_4_False_64]_[0.0005_256]")
    ...
    parser.add_argument('--feature', nargs='+', default=['w2v-msp', 'apvit'],
                        help='Specify the features used (only one).')
    parser.add_argument('--checkpoint_seed', required=False,
                        help='Checkpoints to use, e.g. '
    ...
    parser.add_argument('--keep_checkpoints', action='store_true', default=True,help='Set this in order *not* to delete all the '
    ...
    if args.predict:
        if args.eval_personalised:
            ...
            args.paths['predict'] = os.path.join(config.PREDICTION_FOLDER, PERSONALISATION, 'personalised', args.emo_dim,
                                                 args.model_id, args.run_name.replace(args.model_id,""))
...
def create_data_loaders(data, test_ids):
    ...
    for subj_data in data:
        data_loader = {}
        for partition in subj_data[0].keys():
...
def eval_trained_checkpoints(paths, feature, emo_dim, normalize, win_len, hop_len, cp_dir, use_gpu):
    ...
    test_ids = None
    for name in feature:
        data_, test_ids = load_personalisation_data(paths, name, emo_dim, normalize=normalize, win_len=win_len,
    ...
    initial_model.eval()
    # personalised_cps = {os.path.splitext(os.path.basename(cp))[0].split("_")[1]:initial_cp for cp in checkpoints}
...
if __name__ == '__main__':
    ...
    if not args.eval_personalised:
        ...
        if args.result_csv:
            ...
            _,_,devel_df = d
            devel_predict_csv = os.path.join(args.paths['predict'], 'predictions_devel.csv')
            devel_df.to_csv(devel_predict_csv, index=False)

...
class MuSeDataset(Dataset):
    def __init__(self, data, partition):
        ...
        self.feature_dim = sum([feature[0].shape[-1] for feature in features])
        self.n_samples = len(features[0])

        label_lens = []
        feature_len = []
        for feature_item in features[0]:
        ...
        max_feature_len = np.max(np.array(feature_len))
        self.feature_len_in_one_step = max_feature_len
        max_label_len = np.max(np.array(label_lens))
        # if max_label_len > 1:
        #     assert(max_feature_len==max_label_len)

        self.feature_len = torch.tensor(feature_len)

        features = [[np.pad(f, pad_width=((0, max_feature_len-f.shape[0]),(0,0))) for f in features[i]] for i in range(len(features))]
        self.features = [torch.tensor(np.concatenate(f,axis = 1), dtype=torch.float) for f in zip(*features)]
    ...
    def __getitem__(self, idx):
        ...
        feature_len = self.feature_len[idx]
